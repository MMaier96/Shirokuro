...
public strictfp class MersenneTwisterFast extends Random implements Serializable, Cloneable {
	 ...
	 * internal state. Otherwise, disregard this method. See stateEquals(other).
	 */
	...
	/**
	 * Returns a random double in the half-open range from [0.0,1.0). Thus 0.0 is a
	 * valid result but 1.0 is not.
	 */
	 ...
	 * a valid result but 1.0f is not.
	 */
	 ...
	 * greater than 0, or an IllegalArgumentException is raised.
	 */
	...
	/**
	 * Returns true if the random.MersenneTwisterFast's current internal state is
	 * equal to another random.MersenneTwisterFast. This is roughly the same as
	 * equals(other), except that it compares based on value but does not guarantee
	 * the contract of immutability (obviously random number generators are
	 * immutable). Note that this does NOT check to see if the internal gaussian
	 * storage is the same for both. You can guarantee that the internal gaussian
	 * storage is the same (and so the nextGaussian() methods will return the same
	 * values) by calling clearGaussian() on both objects.
	 */
